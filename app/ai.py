...
评分尺子：
- 0.00 到 0.15：乱码、纯符号、明显错误词汇。
- 0.16 到 0.32：大概率拒绝，例如“常用词 + 单个助词”、“常用词 + 语气词”、不完整的短语、自然但不适合作为词库词条的残片。
- 0.33 到 0.66：边界案例、语义不完整但不能确定错误、或你确实拿不准。
- 0.67 到 0.85：自然短语、可用词条、无明显错误的中短表达、自然句子。
- 0.86 到 1.00：非常稳定的词汇、单字、专有名词、常见自然短语、姓名。

请重点遵守这些规则：
1. 严厉拒绝明显错误词汇、乱码、纯符号。
2. 严厉拒绝“常用词 + 单个助词”这类不适合作为词库词条的表达，例如“喝水了”“吃饭了”“喝水啊”。
3. 拒绝语义不完整的短语，例如“项目已经”“厕所然后”。
4. 接受单个字，倾向接受两个字或更多字的稳定词汇。
5. 倾向接受自然、无明显语法错误的短语和句子。
6. 合法、自然的中英混合词条可以接受，不应仅因包含英文、缩写、品牌名或专有名词就拒绝。例如“OpenAI助手”“ChatGPT插件”“Python脚本”通常可以接受。
7. 只有在中英混合形式明显混乱、像乱码、乱拼或不自然残片时，才应拒绝。
8. 不要为了凑结果而极端自信；拿不准时请给中间分。
9. candidates 中如果包含 weight，表示词频已被明确定义；词频越高，通常说明越常用、越适合作为输入法词条，可适度提高接受倾向。
10. candidates 中没有 weight 时，表示词频未定义；不要把“未定义”理解成低频或不常用。
11. 词频只是辅助参考，不能覆盖上述硬规则；即使词频很高，单字、明显错误词汇、乱码、以及“常用词 + 单个助词/语气词”仍应拒绝。
